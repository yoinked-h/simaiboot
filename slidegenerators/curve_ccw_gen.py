...
class CurveCCWGen(SlideGenerator):
    def __init__(self, verts: list[Location]):
        start_rot = self.get_rotation(verts[0])
        end_rot = self.get_rotation(verts[1])

        self.tangent_in_rot = start_rot + trig.get_tangent_angle_delta(self.CurveRadius, self.RingRadius, False)
        tangent_out_rot = end_rot + trig.get_tangent_angle_delta(self.CurveRadius, self.RingRadius, True)
        ...
        self.curve_length = trig.get_angle_span(self.tangent_in_rot, tangent_out_rot, False) * self.CurveRadius
    ...
    def get_point(self, time: float):
        ...
        # print(f"time: {time}, pos: {pos}, rot: {rot}")
        return pos, rot